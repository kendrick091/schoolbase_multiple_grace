...
document.getElementById("submitSubjects").addEventListener("click", async function () {
  ...
  const tx = db.transaction("thirdTerm", "readwrite");
...
async function displayTable() {
  ...
  const transaction = db.transaction(["thirdTerm", "subjectStore"], "readonly");
  const thirdTermStore = transaction.objectStore("thirdTerm");
  const subjectStore = transaction.objectStore("subjectStore");

  const tbody = document.querySelector("#student-thirdTerm tbody");
  tbody.innerHTML = "";
